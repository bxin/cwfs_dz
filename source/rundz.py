...
def runcwfs(obsID, eimage, isnap, I1Field, I2Field, inst, algo, model):
    if eimage == 0:
        I1File = 'image/wfe_%s_%03d_0.fits'%(obsID, isnap)
        I2File = 'image/wfe_%s_%03d_1.fits'%(obsID, isnap)
    else:
        I1File = 'image/wfs_%s_%03d_0.fits'%(obsID, isnap)
        I2File = 'image/wfs_%s_%03d_1.fits'%(obsID, isnap)
...
def runPhosim(obsID, dz, instFile, cmdFile, nsnap, filter, field, eimage,
    ...
    try:
        ...
        runProgram('cp data/focalplanelayout_%2d.txt %s/focalplanelayout.txt'%(
            dz*10, iscDir))
        runProgram('cp data/segmentation_%2d.txt %s/segmentation.txt'%(
            dz*10, iscDir))
    ...
    for itra in range(2):
        ...
        for isnap in range(nsnap):
            ...
            if eimage == 1:
                ...
                amp = IHDU[0].data  
                IHDU.close()
                stamp = amp[ampCenter[1]-stampSize/2:ampCenter[1]+stampSize/2,
...
def createPertFiles(obsID, nsnap, mag, debugLevel):
    ...
    dz, r0seeing, seed, teleState, filter, field = parseObsID(obsID, debugLevel)

    # create inst file
    source = 'data/wfs_%s.inst'%obsID
...
def parseObsID(obsID, debugLevel):
    ...
    return dz, r0seeing, seed, teleState, filter, field
